...
utf8_char_validity validate_utf8_char(const char* str, size_t offset) {
    ...
    if (((uint8_t)str[offset] & B10000000) == B00000000)
        return (utf8_char_validity) { .valid = true, .next_offset = offset + 1 };

    // Two-byte UTF-8 characters have the form 110xxxxx 10xxxxxx
    if (((uint8_t)str[offset + 0] & B11100000) == B11000000 &&
        ((uint8_t)str[offset + 1] & B11000000) == B10000000) {
        ...
        // 110(00001) 10(111111)
        // 110(00010) 10(000000)
        if (((uint8_t)str[offset] & B00011111) < B00000010)
    ...
    if (((uint8_t)str[offset + 0] & B11110000) == B11100000 &&
        ((uint8_t)str[offset + 1] & B11000000) == B10000000 &&
        ((uint8_t)str[offset + 2] & B11000000) == B10000000) {
        ...
        if (((uint8_t)str[offset + 0] & B00001111) == B00000000 &&
            ((uint8_t)str[offset + 1] & B00111111) < B00100000)
            return (utf8_char_validity) { .valid = false, .next_offset = offset };

        // Reject UTF-16 surrogates
        // U+D800 to U+DFFF
        // 1110(1101) 10(100000) 10(000000) ED A0 80 to 1110(1101) 10(111111) 10(111111) ED BF BF
        if ((uint8_t)str[offset + 0] == B11101101 &&
            (uint8_t)str[offset + 1] >= B10100000 &&
            (uint8_t)str[offset + 1] <= B10111111)
            return (utf8_char_validity) { .valid = false, .next_offset = offset };
    ...
    if (((uint8_t)str[offset + 0] & B11111000) == B11110000 &&
        ((uint8_t)str[offset + 1] & B11000000) == B10000000 &&
        ((uint8_t)str[offset + 2] & B11000000) == B10000000 &&
        ((uint8_t)str[offset + 3] & B11000000) == B10000000) {
        ...
        if (((uint8_t)str[offset + 0] & B00000111) == B00000000 &&
            ((uint8_t)str[offset + 1] & B00111111) < B00010000)
            return (utf8_char_validity) { .valid = false, .next_offset = offset };
...
owned_utf8_string make_utf8_string_lossy(const char* str) {
    ...
    while (offset < len) {
        ...
        if (char_validity.valid) {
            ...
            //      = EF BF BD
            buffer[buffer_offset++] = (char)0xEF;
            buffer[buffer_offset++] = (char)0xBF;
            buffer[buffer_offset++] = (char)0xBD;
            offset++;
        }
    }
...
bool is_utf8_char_boundary(const char* str) {
    return (uint8_t)*str <= B01111111 || (uint8_t)*str >= B11000000;
}
...
uint32_t unicode_code_point(utf8_char uchar) {
    ...
    case 1: return uchar.str[0] & B01111111;
    case 2: return
        (uchar.str[0] & B00011111) << 6 |
        (uchar.str[1] & B00111111);
    case 3: return
        (uchar.str[0] & B00001111) << 12 |
        (uchar.str[1] & B00111111) << 6 |
        (uchar.str[2] & B00111111);
    case 4: return
        (uchar.str[0] & B00000111) << 18 |
        (uchar.str[1] & B00111111) << 12 |
        (uchar.str[2] & B00111111) << 6 |
        (uchar.str[3] & B00111111);
    }
